...
// Multi-profile support added manually — see --profile flag and ensureRuntime().
...
const generatorInfo = "Generated by google-workspace-mcp-cli@0.1.0 — https://github.com/steipete/mcporter";
...
const CONFIG_ROOT = path.join(os.homedir(), '.google-workspace-mcp');
const PROFILES_DIR = path.join(CONFIG_ROOT, 'profiles');

/**
 * Ensure the profile directory exists and contains the right structure.
 * Each profile has its own fake HOME directory:
 *   ~/.google-workspace-mcp/profiles/{profile}/home/.google-workspace-mcp/
 *
 * On first run, migrate the legacy root token.json/credentials.json to the
 * gmail profile so existing users are not broken.
 */
function ensureProfileDir(profile: string): string {
  const profileFakeHome = path.join(PROFILES_DIR, profile, 'home');
  const profileMcpDir = path.join(profileFakeHome, '.google-workspace-mcp');
  fs.mkdirSync(profileMcpDir, { recursive: true });

  // Migration: on first run, move root token.json + credentials.json into
  // the gmail profile so existing usage is preserved without re-auth.
  if (profile === 'gmail') {
    const rootToken = path.join(CONFIG_ROOT, 'token.json');
    const rootCreds = path.join(CONFIG_ROOT, 'credentials.json');
    const destToken = path.join(profileMcpDir, 'token.json');
    const destCreds = path.join(profileMcpDir, 'credentials.json');
    const migrationFlag = path.join(CONFIG_ROOT, '.migrated-to-profiles');

    if (!fs.existsSync(migrationFlag)) {
      // Copy (not move) so the root files stay usable for other tooling
      if (fs.existsSync(rootToken) && !fs.existsSync(destToken)) {
        fs.copyFileSync(rootToken, destToken);
      }
      if (fs.existsSync(rootCreds) && !fs.existsSync(destCreds)) {
        fs.copyFileSync(rootCreds, destCreds);
      }
  ...
  return profileFakeHome;
}

/**
 * Build env overrides for the MCP server subprocess so it looks for
 * token/credentials in the profile-specific directory.
 * os.homedir() in the child process reads USERPROFILE on Windows, HOME on Unix.
 */
function profileEnv(profile: string): Record<string, string> {
  const fakeHome = ensureProfileDir(profile);
  return {
...
// ---------------------------------------------------------------------------
// Send-As support (--from flag for send-email)
// Implemented via direct Gmail REST API call — the MCP server does not support
// the `from` header, so we bypass it when --from is provided.
// ---------------------------------------------------------------------------

async function refreshTokenIfNeeded(tokenPath: string, credsPath: string): Promise<string> {
  ...
  const now = Date.now();
  const isExpired = token.expiry_date ? token.expiry_date < now + 60_000 : false;
  ...
  if (!token.refresh_token) {
    throw new Error('Token is expired and no refresh_token available. Please re-authenticate.');
  }
  ...
  const clientCreds = creds.installed ?? creds.web;
  if (!clientCreds) throw new Error('Invalid credentials.json format');
  ...
  const newToken = await resp.json() as {
    access_token: string;
    expires_in?: number;
  };
  ...
  const updated = {
    ...
    expiry_date: newToken.expires_in ? now + newToken.expires_in * 1000 : undefined,
  };
  fs.writeFileSync(tokenPath, JSON.stringify(updated, null, 2));
  return newToken.access_token;
}

async function sendEmailWithFrom(opts: {
  from: string;
  to: string;
  subject: string;
  body: string;
  ...
  const { from, to, subject, body, cc, bcc, profile } = opts;

  const fakeHome = ensureProfileDir(profile);
  const mcpDir = path.join(fakeHome, '.google-workspace-mcp');
  const tokenPath = path.join(mcpDir, 'token.json');
  const credsPath = path.join(mcpDir, 'credentials.json');

  if (!fs.existsSync(tokenPath)) {
    throw new Error(`No token found for profile '${profile}'. Run without --from first to authenticate.`);
  }

  const accessToken = await refreshTokenIfNeeded(tokenPath, credsPath);
  ...
  const encodedEmail = Buffer.from(headers)
    .toString('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=+$/, '');
...
// ---------------------------------------------------------------------------
// Read-email fix: recursive body extraction via direct Gmail REST API.
// The MCP server's read_email only scans top-level parts — multipart/alternative
// and multipart/mixed emails nest text/plain inside a container part, so the
// flat scan finds nothing and falls back to message.data.snippet (~200 chars).
// This direct implementation recursively walks all parts to find the full body.
// ---------------------------------------------------------------------------
...
/** Recursively walk a MIME part tree and collect all text/plain body data. */
...
/** Recursively walk a MIME part tree and collect all text/html body data (fallback). */
function extractHtmlText(part: GmailMessagePart): string {
  if (part.mimeType === 'text/html' && part.body?.data) {
    return Buffer.from(part.body.data, 'base64').toString('utf-8');
  }
  if (part.parts && part.parts.length > 0) {
    return part.parts.map(extractHtmlText).join('');
  }
  return '';
}

async function readEmailDirect(opts: {
  messageId: string;
  profile: string;
}): Promise<Record<string, unknown>> {
  const { messageId, profile } = opts;
  ...
  if (!fs.existsSync(tokenPath)) {
    throw new Error(`No token found for profile '${profile}'. Please authenticate first.`);
  }
  ...
  const message = await resp.json() as GmailMessage;
  const headers = message.payload?.headers ?? [];
  const getHeader = (name: string) =>
    headers.find(h => h.name?.toLowerCase() === name.toLowerCase())?.value ?? '';

  // Attempt plain text first, then HTML, then snippet as last resort.
  const payload = message.payload ?? {};
  let body = extractPlainText(payload);
  if (!body) {
    body = extractHtmlText(payload);
  }
  if (!body) {
...
program.option('--profile <profile>', 'Google account profile to use (default: gmail)', 'gmail');
...
program
	...
	.alias("read_email")	.action(async (cmdOpts) => {
		const globalOptions = program.opts();
		// Use direct Gmail REST API to avoid the MCP server's flat part scan,
		// which falls back to the ~200-char snippet for multipart/alternative emails.
		const result = await readEmailDirect({
			messageId: cmdOpts.messageId,
			profile: globalOptions.profile ?? 'gmail',
		});
		printResult(result, globalOptions.output ?? 'text');
	})
...
program
	.command("send-email")
	.summary("send-email --to <to> --subject <subject> --body <body> [--from <from>] [--cc <cc>] [--bcc <bcc>] [--raw <json>]")
	.description("Send a new email (supports optional file attachments and Send-As aliases via --from)")
	.usage("--to <to> --subject <subject> --body <body> [--from <from>] [--cc <cc>] [--bcc <bcc>] [--raw <json>]")
	.option('--raw <json>', 'Provide raw JSON arguments to the tool, bypassing flag parsing.')

	.requiredOption("--to <to>", "Recipient email address")
	.requiredOption("--subject <subject>", "Email subject")
	.requiredOption("--body <body>", "Email body text")
	.option("--from <from>", "Send-As alias address (e.g. alan@goshootless.com). Must be configured as a Gmail Send-As alias.")
	...
	.alias("send_email")	.action(async (cmdOpts) => {
		const globalOptions = program.opts();
		const profile: string = globalOptions.profile ?? 'gmail';

		// --from triggers direct Gmail API path (MCP server doesn't support Send-As)
		if (cmdOpts.from) {
			await sendEmailWithFrom({
				from: cmdOpts.from,
				to: cmdOpts.to,
	...
	.addHelpText('afterAll', () => '\n' + "// optional (3): from, attachmentPaths, attachments" + '\n');
...
async function ensureRuntime(): Promise<Awaited<ReturnType<typeof createRuntime>>> {
	...
	const serverDef: ServerDefinition = {
		...(normalizeEmbeddedServer(embeddedServer) as unknown as ServerDefinition),
